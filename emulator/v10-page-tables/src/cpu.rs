...
impl Cpu {
            ...
            0x03 => {
                ...
                match funct3 {
                    ...
                    _ => {
                        ...
                        return Err(RvException::IllegalInstruction(inst));
                    }
                }
            }
            0x0f => {
                        ...
                        match (rs2, funct7) {
                            (0x0, 0x0) => {
                                ...
                                match self.mode {
                                    User => {
                                        self.update_pc()?;
                                        return Err(EnvironmentCallFromUmode(self.pc));
                                    }
                                    Supervisor => {
                                        self.update_pc()?;
                                        return Err(EnvironmentCallFromSmode(self.pc));
                                    }
                                    Machine => {
                                        self.update_pc()?;
                                        return Err(EnvironmentCallFromMmode(self.pc));
                                    }
                                    _ => unreachable!(),
                                }
                            }
                            ...
                            (0x2, 0x8) => {
                                ...
                                self.mode = match (self.csr.load(SSTATUS) >> 8) & 1 {
                                    1 => Supervisor,
                                    _ => User,
                                };
                                ...
                                self.csr.store(
                                    SSTATUS,
                                    if ((self.csr.load(SSTATUS) >> 5) & 1) == 1 {
                                        self.csr.load(SSTATUS) | (1 << 1)
                                    } else {
                                        self.csr.load(SSTATUS) & !(1 << 1)
                                    },
                                );
                            ...
                            (0x2, 0x18) => {
                                ...
                                self.mode = match (self.csr.load(MSTATUS) >> 11) & 0b11 {
                                    2 => Machine,
                                    1 => Supervisor,
                                    _ => User,
                                };
                                ...
                                self.csr.store(
                                    MSTATUS,
                                    if ((self.csr.load(MSTATUS) >> 7) & 1) == 1 {
                                        self.csr.load(MSTATUS) | (1 << 3)
                                    } else {
                                        self.csr.load(MSTATUS) & !(1 << 3)
                                    },
                                );
    ...
    pub fn execute2(&mut self, inst: u64) -> Result<(), RvException> {
